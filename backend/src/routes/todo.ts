...
/* 
    ...
    server.get('/todos', opts, async(request, reply)=>{
        try {
            ...
            console.log(`Get /todos Erroe: ${error}`)
            return reply.status(500).send(`[Server Error]: ${error}`)
        }
    })
    ...
    server.post('/todos', opts, async(request,reply)=>{
        try{
            ...
            console.log(`Post /todos Erroe: ${error}`)
            return reply.status(500).send(`[Server Error]: ${error}`)
        }
    })
    //Update
    server.put<{ Params: IdParam}>('/todos/:id', opts, async(request,reply)=>{
        try{
            ...
            const todo:ITodo = await todoRepo.updateTodo(id,todoBody) as ITodo
            return reply.status(201).send( {todo} )
        }catch (error){
            console.log(`Post /todos Erroe: ${error}`)
            return reply.status(500).send(`[Server Error]: ${error}`)
    ...
    server.delete<{ Params: IdParam}>('/todos/:id', opts, async(request,reply)=>{
        try{
            const id:string = request.params.id
            const todo:ITodo = await todoRepo.deleteTodo(id) as ITodo
            return reply.status(201).send( {todo} )
        }catch (error){
            console.log(`Post /todos Erroe: ${error}`)
            return reply.status(500).send(`[Server Error]: ${error}`)
